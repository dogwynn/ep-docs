...
python serve.py              # Opens Person Browser at http://localhost:8080
python serve.py --network    # Opens Network Viewer instead

# Preprocess data for the person browser (run once after extraction)
...
### Web Applications
- `serve.py` - HTTP server with API endpoints (port 8080)
- `person_browser.html` - Interactive person search and profile viewer
- `network_viewer.html` - Sigma.js/Graphology-based WebGL network visualization
- `layout-worker.js` - Web Worker for background force-directed layout calculation
- `preprocess_browser_data.py` - Generates indexed data for person browser → `data/`
...
- `preprocess_browser_data.py`: `MAX_QUOTES_PER_PERSON` (default 100), `MAX_DOCUMENTS_PER_PERSON` (default 200)
- `serve.py`: `PORT` (default 8080), `--network` flag to open network viewer
...
- `data/` - Person browser indexed data (generated by `preprocess_browser_data.py`):
  - `person_index.json` - Search index with all persons and aliases
  - `persons/` - Individual JSON files per person containing:
    - Associates, organizations, locations, timeline
    - Quotes (up to 100 per person)
